      ...
      <div class="table-responsive">
        <table class="table table-striped table-sm">
          <thead>
	        ...
	        <c:forEach items="${cList }" var="report" varStatus="i">
				<tr>
					<td>${i.count }</td>
					<td>${report.reportDate }</td>
					<td>${report.memberEmail }</td>
			...
			<c:if test="${!empty cList }">
				<tr align="center" height="20">
					<td colspan="6"><c:if test="${currentPage != 1 }">
							<a href="/report/reportComment?page=${currentPage - 1 }">[이전]</a>
						</c:if> <c:forEach var="p" begin="${startNavi }" end="${endNavi }">
							<c:if test="${currentPage eq p }">
								<b>${p }</b>
							</c:if>
							<c:if test="${currentPage ne p }">
								<a href="/report/reportComment?page=${p }">${p }</a>
							</c:if>
						</c:forEach> <c:if test="${maxPage > currentPage }">
							<a href="/report/reportComment?page=${currentPage + 1 }">[다음]</a>
						</c:if></td>
				</tr>
			</c:if>
			</td>
