	...
	<section class="container-md" id="">
		...
		<div id="main-contents" class="">
			...
			<div id="serach-title-area">
				<div>
					<div id="laveltitle">
						<c:if test="${searchCondition ne 'no' }">
							<span class="titleLavel">${selectHangul}</span>
						</c:if>
						<!-- search기준 -->
						<span class="titleLavel">${searchValue}</span>
						<!-- 검색어 -->
						<c:if test="${hachConditionHangul ne 'no' }">
							<span class="titleLavel">${hachConditionHangul}</span>
						</c:if>
						<!-- 해시태그 -->
